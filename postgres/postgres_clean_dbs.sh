...
DROPDB=/usr/pgsql-9.2/bin/dropdb
...
for oid in `ls -1 ${PGDATA}/base/ | grep -v pgsql_tmp`;do
  ...
  if [[ $t1 -lt $t2 ]]; then
    ...
    if [[ ${dbname} != "" ]]; then
      echo "cleaning DB: $dbname, last accessed on: $t1"
...
EOD
      RET=$?
	  if [[ ${RET} != 0 ]]; then
        echo "psql failed for ${dbname} - please check logs"
	    exit 1
      fi
      #${DROPDB} ${dbname};
	  RET=$?
	  if [[ ${RET} != 0 ]]; then
        echo "dropdb failed for ${dbname} - please check logs"
	    exit 1
      fi
     fi
  else

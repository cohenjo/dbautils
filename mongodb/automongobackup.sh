...
if [ "$DBUSERNAME" ]; then
	...
	AUTH_OPT=" --username=$DBUSERNAME --password=$DBPASSWORD --authenticationDatabase=$DBAUTHDB "
fi
...
function select_secondary_member {
    ...
    members=( $(mongo $AUTH_OPT --quiet --host $DBHOST:$DBPORT --eval 'rs.conf().members.forEach(function(x){ print(x.host) })') )

    # Check each replset member to see if it's a secondary and return it.
    if [ ${#members[@]} -gt 1 ]; then
        for member in "${members[@]}"; do

            is_secondary=$(mongo $AUTH_OPT --quiet --host $member --eval 'rs.isMaster().secondary')
